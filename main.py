...
JOIN_TO_CREATE_CHANNEL_ID = 1433175005558210720  # Replace with your channel ID

async def log_event(event_message):
    print(event_message)
    with open("activity_logs.txt", "a") as log_file:
        log_file.write(f"{time.ctime()} - {event_message}\n")
    channel = bot.get_channel(1433178706637230252)  # Replace with your log channel ID
    if channel:
...
async def on_interaction(interaction: discord.Interaction):
    ...
    try:
        result = await actions[custom_id](interaction)
        if result is None:
